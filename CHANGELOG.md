...
- **Image delivery no longer interrupts scrubbing**: full images now cross to the view as binary instead of base64 (a third smaller, no more multi-MB string garbage — traces showed 10-22ms main-thread stalls per arrival plus GC pauses), and while the user is stepping fast, payloads for off-screen tuples wait for a quiet moment, then drain one per tick; the current tuple's images are never delayed
- **Changing tuples now resets zoom/pan by default**: the previous always-persist behavior is available as `imageCompare.keepZoomOnTupleChange`; modality switches within a tuple keep the view either way
- **Carousel and status-bar polish for many-modality sessions**: the carousel opens wide enough to show every modality column and its resize limit now scales with the modality count (tiles shrink continuously to fit narrower widths instead of clipping columns); resizing is smooth (one CSS variable instead of a style write per tile, frame-coalesced) and keeps the current row centered — first row pinned top, last pinned bottom; stepping tuples keeps the tile grid pixel-stationary — the wall jumps exactly one row height with no animation, so only tile content and the highlight change (every animated variant, native or custom, showed residual jitter) — with a custom scrollbar thumb in a reserved gutter and free wheel/thumb scrolling; the carousel DOM is virtualized (a recycled pool of ~35 rows instead of one element per tuple), making resize truly interactive and cutting open-time DOM sixty-fold; the zoom readout can no longer be squeezed out of the status bar by wide pills; the in-app shortcut legend (?) caught up with the README table
- **Save Session As** (`Ctrl+S` or the title-bar save icon): saves a copy of the `.imagecompare` file — right-click comparisons live in a pruned cache, and this is the way to keep one. The dialog defaults to the comparison's natural home (base dir / common parent), paths are written relative when the file lands in the compared folders' parent (absolute otherwise), a results sidecar is copied along, and saved files now carry a `version` field so future format changes fail loudly on old builds instead of half-opening
