 ...
 *   - test-dashboard.html  (colored, grouped, with a summary)
 *   - FEATURES.md          (markdown table fallback)
 *
 * Status per feature:
...
if (!reuse) {
  ...
  runCaptureStdout('npx vitest run --reporter=json', UNIT_JSON);
  checked('unit', UNIT_JSON);
  run('npx playwright test --reporter=json', { PLAYWRIGHT_JSON_OUTPUT_NAME: WEBVIEW_JSON });
...
writeFileSync(join(ROOT, 'test-dashboard.html'), html);
