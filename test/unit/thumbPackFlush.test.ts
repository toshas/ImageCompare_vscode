...
afterAll(async () => {
  // `dispose()` is fire-and-forget by contract, so let its last async step finish before the bed
  // disappears — otherwise the teardown, not the code, is what makes the write fail.
